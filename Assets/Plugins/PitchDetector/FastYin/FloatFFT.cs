...
public class FloatFFT
{
    ...
    private static readonly int[] factors = { 4, 2, 3, 5 };

    private static readonly float PI = 3.14159265358979311599796346854418516f;

    private static readonly float TWO_PI = 6.28318530717958623199592693708837032f;
    ...
    public FloatFFT(int n)
    {
        ...
        if (!ConcurrencyUtils.isPowerOf2(n))
        {
            if (getReminder(n, factors) >= 211)
            {
                ...
                this.ip = new int[2 + (int)Mathf.Ceil(2 + (1 << (int)(Mathf.Log(nBluestein + 0.5f) / Mathf.Log(2)) / 2))];
        ...
        else
        {
            ...
            this.ip = new int[2 + (int)Mathf.Ceil(2 + (1 << (int)(Mathf.Log(n + 0.5f) / Mathf.Log(2)) / 2))];
            this.w = new float[n];
            int twon = 2 * n;
            nw = ip[0];
            if (twon > (nw << 2))
     ...
     */
    public void complexInverse(float[] a, bool scale)
    {
     ...
     *            if true then scaling is performed
     */
    public void complexInverse(float[] a, int offa, bool scale)
    {
        if (n == 1)
            return;
        switch (plan)
    ...
    for (k1 = 1; k1 <= nf; k1++)
    {
        ...
        for (j = 1; j <= ipm; j++)
        {
            ...
            for (ii = 4; ii <= idot; ii += 2)
            {
                ...
                arg = fi * argld;
                int idx = i + twon;
                wtable[idx - 1] = (float)Mathf.Cos(arg);
                wtable[idx] = (float)Mathf.Sin(arg);
            }
            if (ip > 5)
            {
                int idx1 = i1 + twon;
    ...
    for (k1 = 1; k1 <= nfm1; k1++)
    {
        ...
        for (j = 1; j <= ipm; ++j)
        {
            ...
            for (ii = 3; ii <= ido; ii += 2)
            {
                ...
                arg = fi * argld;
                int idx = i + n;
                wtable_r[idx - 2] = (float)Mathf.Cos(arg);
...
private void bluesteini()
{
    ...
    for (int i = 1; i < n; i++)
    {
        ...
        if (k >= 2 * n)
            k -= 2 * n;
        arg = pi_n * k;
        bk1[2 * i] = (float)Mathf.Cos(arg);
        bk1[2 * i + 1] = (float)Mathf.Sin(arg);
    }
...
private void makewt(int nw)
{
    ...
    if (nw > 2)
    {
        ...
        delta2 = delta * 2;
        wn4r = (float)Mathf.Cos(delta * nwh);
        w[0] = 1;
        w[1] = wn4r;
        if (nwh == 4)
        {
            w[2] = (float)Mathf.Cos(delta2);
            w[3] = (float)Mathf.Sin(delta2);
        }
        else if (nwh > 4)
        {
            makeipt(nw);
            w[2] = (float)(0.5 / Mathf.Cos(delta2));
            w[3] = (float)(0.5 / Mathf.Cos(delta * 6));
            for (j = 4; j < nwh; j += 4)
            {
                deltaj = delta * j;
                deltaj3 = 3 * deltaj;
                w[j] = (float)Mathf.Cos(deltaj);
                w[j + 1] = (float)Mathf.Sin(deltaj);
                w[j + 2] = (float)Mathf.Cos(deltaj3);
                w[j + 3] = (float)-Mathf.Sin(deltaj3);
            }
        }
        nw0 = 0;
        while (nwh > 2)
...
private void makect(int nc, float[] c, int startc)
{
    ...
    if (nc > 1)
    {
        ...
        c[startc] = (float)Mathf.Cos(delta * nch);
        c[startc + nch] = (float)(0.5 * c[startc]);
        for (j = 1; j < nch; j++)
        {
            deltaj = delta * j;
            c[startc + j] = (float)(0.5 * Mathf.Cos(deltaj));
            c[startc + nc - j] = (float)(0.5 * Mathf.Sin(deltaj));
        }
    }
}
...
void passf2(int ido, int l1, float[] _in, int in_off, float[] _out, int out_off, int offset, int isign)
{
    float t1i, t1r;
    int iw1;
    iw1 = offset;
    ...
    if (ido <= 2)
    {
        for (int k = 0; k < l1; k++)
        {
            ...
            int iidx2 = iidx1 + ido;
            float a1r = _in[iidx1];
                float a1i = _in[iidx1 + 1];
                float a2r = _in[iidx2];
                float a2i = _in[iidx2 + 1];

                int oidx1 = out_off + idx0;
int oidx2 = oidx1 + idx;
                _out[oidx1] = a1r + a2r;
                _out[oidx1 + 1] = a1i + a2i;
                _out[oidx2] = a1r - a2r;
                _out[oidx2 + 1] = a1i - a2i;
            }
        } else {
            for (int k = 0; k<l1; k++) {
...
int iidx2 = iidx1 + ido;
float i1r = _in[iidx1];
                    float i1i = _in[iidx1 + 1];
                    float i2r = _in[iidx2];
                    float i2i = _in[iidx2 + 1];
...
int oidx2 = oidx1 + idx;
                    _out[oidx1] = i1r + i2r;
                    _out[oidx1 + 1] = i1i + i2i;
                    _out[oidx2] = w1r* t1r - w1i* t1i;
                    _out[oidx2 + 1] = w1r* t1i + w1i* t1r;
                }
            }
        }
    }
    ...
    /*----------------------------------------------------------------------
       ...
       isign is +1 for backward and -1 for forward transforms
      ----------------------------------------------------------------------*/
    void passf3(int ido, int l1, float[] _in, int in_off, float[] _out, int out_off, int offset, int isign)
{
    float taur = -0.5f;
...
int oidx3 = oidx2 + idxt;
                _out[oidx1] = _in[iidx3] + tr2;
                _out[oidx1 + 1] = i3i + ti2;
                _out[oidx2] = cr2 - ci3;
                _out[oidx2 + 1] = ci2 + cr3;
                _out[oidx3] = cr2 + ci3;
                _out[oidx3 + 1] = ci2 - cr3;
            }
        } else {
            for (int k = 1; k <= l1; k++) {
...
int oidx3 = oidx2 + idxt;
                    _out[oidx1] = a3r + tr2;
                    _out[oidx1 + 1] = a3i + ti2;
                    _out[oidx2] = w1r* dr2 - w1i* di2;
                    _out[oidx2 + 1] = w1r* di2 + w1i* dr2;
                    _out[oidx3] = w2r* dr3 - w2i* di3;
                    _out[oidx3 + 1] = w2r* di3 + w2i* dr3;
                }
            }
        }
    }
    ...
    /*----------------------------------------------------------------------
       ...
       isign is +1 for backward and -1 for forward transforms
      ----------------------------------------------------------------------*/
    void passf4(int ido, int l1, float[] _in, int in_off, float[] _out, int out_off, int offset, int isign)
{
...
int oidx4 = oidx3 + idx0;
                _out[oidx1] = tr2 + tr3;
                _out[oidx1 + 1] = ti2 + ti3;
                _out[oidx2] = tr1 + isign* tr4;
                _out[oidx2 + 1] = ti1 + isign* ti4;
                _out[oidx3] = tr2 - tr3;
                _out[oidx3 + 1] = ti2 - ti3;
                _out[oidx4] = tr1 - isign* tr4;
                _out[oidx4 + 1] = ti1 - isign* ti4;
            }
        } else {
            for (int k = 0; k<l1; k++) {
                int idx1 = k * ido;
...
int oidx4 = oidx3 + idx0;
                    _out[oidx1] = tr2 + tr3;
                    _out[oidx1 + 1] = ti2 + ti3;
                    _out[oidx2] = w1r* cr2 - w1i* ci2;
                    _out[oidx2 + 1] = w1r* ci2 + w1i* cr2;
                    _out[oidx3] = w2r* cr3 - w2i* ci3;
                    _out[oidx3 + 1] = w2r* ci3 + w2i* cr3;
                    _out[oidx4] = w3r* cr4 - w3i* ci4;
                    _out[oidx4 + 1] = w3r* ci4 + w3i* cr4;
                }
            }
        }
    }
    ...
    /*----------------------------------------------------------------------
       ...
       isign is +1 for backward and -1 for forward transforms
      ----------------------------------------------------------------------*/
    void passf5(int ido, int l1, float[] _in, int in_off, float[] _out, int out_off, int offset, int isign)
...
int oidx5 = oidx4 + idx0;
                _out[oidx1] = i3i + tr2 + tr3;
                _out[oidx1 + 1] = i3r + ti2 + ti3;
                _out[oidx2] = cr2 - ci5;
                _out[oidx2 + 1] = ci2 + cr5;
                _out[oidx3] = cr3 - ci4;
                _out[oidx3 + 1] = ci3 + cr4;
                _out[oidx4] = cr3 + ci4;
                _out[oidx4 + 1] = ci3 - cr4;
                _out[oidx5] = cr2 + ci5;
                _out[oidx5 + 1] = ci2 - cr5;
            }
        } else {
            for (int k = 1; k <= l1; k++) {
...
int oidx5 = oidx4 + idx0;
                    ...
                    _out[oidx3] = w2r* dr3 - w2i* di3;
                    _out[oidx3 + 1] = w2r* di3 + w2i* dr3;
                    _out[oidx4] = w3r* dr4 - w3i* di4;
                    _out[oidx4 + 1] = w3r* di4 + w3i* dr4;
                    _out[oidx5] = w4r* dr5 - w4i* di5;
                    _out[oidx5 + 1] = w4r* di5 + w4i* dr5;
                }
            }
        }
    }
    ...
    /*----------------------------------------------------------------------
       ...
       isign is +1 for backward and -1 for forward transforms
      ----------------------------------------------------------------------*/
    void passfg(int[] nac, int ido, int ip, int l1, int idl1, float[] _in, int in_off, float[] _out, int out_off, int offset, int isign)
    {
        ...
        if (ido >= l1)
        {
            for (int j = 1; j < ipph; j++)
            {
                ...
                for (int k = 0; k < l1; k++)
                {
                    ...
                    {
                        int oidx1 = out_off + i;
                        float i1r = _in[in_off + i + idx1 + idx6];
                            float i2r = _in[in_off + i + idx2 + idx6];
                            _out[oidx1 + idx4] = i1r + i2r;
                            _out[oidx1 + idx5] = i1r - i2r;
                        }
                    }
                }
                for (int k = 0; k<l1; k++) {
    ...
    int idxt2 = idxt1 * ip;
                    for (int i = 0; i<ido; i++) {
                        _out[out_off + i + idxt1] = _in[in_off + i + idxt2];
                    }
                }
            } else {
                for (int j = 1; j<ipph; j++) {
    ...
    int idx3 = out_off + i;
    int idx4 = in_off + i;
    float i1r = _in[idx4 + idxt3 + idx2];
                            float i2r = _in[idx4 + idxt4 + idx2];
                            _out[idx3 + idx1 + idxt1] = i1r + i2r;
                            _out[idx3 + idx1 + idxt2] = i1r - i2r;
                        }
                    }
                }
                for (int i = 0; i<ido; i++) {
                    for (int k = 0; k<l1; k++) {
                        int idx1 = k * ido;
                        _out[out_off + i + idx1] = _in[in_off + i + idx1 * ip];
                    }
                }
            }
    ...
    w1r = wtable[idxt3 - 2];
                ...
                for (int ik = 0; ik<idl1; ik++) {
                    int idx1 = in_off + ik;
    int idx2 = out_off + ik;
                    _in[idx1 + idxt1] = _out[idx2] + w1r* _out[idx2 + idl1];
                    _in[idx1 + idxt2] = w1i* _out[idx2 + idxt0];
                }
                idlj = idl;
                inc += ido;
                for (int j = 2; j<ipph; j++) {
    ...
    int idxt6 = jc * idl1;
                    for (int ik = 0; ik<idl1; ik++) {
                        int idx1 = in_off + ik;
    int idx2 = out_off + ik;
                        _in[idx1 + idxt1] += w2r* _out[idx2 + idxt5];
                        _in[idx1 + idxt2] += w2i* _out[idx2 + idxt6];
                    }
                }
            }
            for (int j = 1; j<ipph; j++) {
    ...
    float i1i = _in[iidx1 - 1];
                    ...
                    int oidx1 = idx3 + idx1;
    int oidx2 = idx3 + idx2;
                    _out[oidx1 - 1] = i1i - i2r;
                    _out[oidx2 - 1] = i1i + i2r;
                    _out[oidx1] = i1r + i2i;
                    _out[oidx2] = i1r - i2i;
                }
            }
            ...
            if (ido == 2)
                return;
            nac[0] = 0;
            Array.Copy(_out, out_off, _in, in_off, idl1);
            int idx0 = l1 * ido;
            for (int j = 1; j<ip; j++) {
                int idx1 = j * idx0;
                for (int k = 0; k<l1; k++) {
    ...
    int iidx1 = in_off + idx2 + idx1;
                    _in[iidx1] = _out[oidx1];
                    _in[iidx1 + 1] = _out[oidx1 + 1];
                }
            }
            if (idot <= l1) {
                idij = 0;
    ...
    int iidx1 = idx3 + idx5;
    int oidx1 = idx4 + idx5;
    float o1i = _out[oidx1 - 1];
                            float o1r = _out[oidx1];
                            _in[iidx1 - 1] = w1r* o1i - w1i* o1r;
                            _in[iidx1] = w1r* o1r + w1i* o1i;
                        }
                    }
                }
            } else {
...
//    }
    ...
    //TODO
    private void scale(float m, float[] a, int offa, bool complex)
    {
